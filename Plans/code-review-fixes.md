...
### 3. [High] `waitAndLink` newest-file fallback can link the wrong transcript and cross-wire two meetings' frontmatter
...
### 4. [High] Transient network failure during token refresh permanently signs the user out and destroys the refresh token
...
### 5. [High] SpeakerTagModal: Enter to pick an autocomplete suggestion on the last speaker row instantly applies the whole modal
